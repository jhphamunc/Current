...
$(document).ready(function () {
    ...
    $('#add-ticker-form').submit(function(e) {
        ...
        $('#new-ticker').val(''); // Correct selector
        updatePrices();
    });
...
function updatePrices() {
    tickers.forEach(function (ticker) {
        $.ajax({
            url: '/get_stock_data',
            type: 'POST',
            data: JSON.stringify({ 'ticker': ticker }), // Ensure correct data format
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (data) {
                // Ensure data.openPrice is not 0 to avoid NaN in changePercent calculation
